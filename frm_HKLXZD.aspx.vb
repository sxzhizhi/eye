...
Partial Class frm_HKLXZD
    ...
    Protected Sub but_LoadData_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles but_LoadData.Click
        ...
        If Session("cMCode") <> "" Then
            rs = cPConn.Execute("select * from DicHKLX where zHKLXCode='" & Session("cMCode") & "'")
            If rs.RecordCount <> 0 Then

                T_code.Text = rs.Fields("zHKLXcode").Value

                T_tslb.Text = rs.Fields("zHKLXName").Value

                Session("cMKey") = T_tslb.Text
            Else
                T_code.Text = GetCode("HKLXZD", "R", Year(Now.Date), Month(Now.Date), Day(Now.Date))
    ...
    Protected Sub But_Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles But_Save.Click
        ...
        rs = cPConn.Execute("select * from DicHKLX where zHKLXcode='" & T_code.Text & "'")
        If rs.RecordCount <> 0 Then

            '判断重覆关键字
            If Session("cMKey") <> T_tslb.Text Then
                re = cPConn.Execute("Select * from DicHKLX where zHKLXName='" & T_tslb.Text & "'")
            ...
            cPConn.Execute("update DicHKLX set zHKLXcode='" & T_code.Text & "',zHKLXName='" & T_tslb.Text & "' where zHKLXcode='" & T_code.Text & "'")

            cPConn.Execute("insert into WaterLog(cUser,cAction,cRemark)values('" & Session("userid") & "','修改户口类型字典数据!','" & T_code.Text & "')")
        ...
        Else
            ...
            sBillCode = GetCode("HKLXZD", "W", Year(Now.Date), Month(Now.Date), Day(Now.Date))

            Me.T_code.Text = sBillCode


            cPConn.Execute("insert into DicHKLX(zHKLXCode,zHKLXName) values('" & sBillCode & "','" & T_tslb.Text & "')")

            cPConn.Execute("insert into WaterLog(cUser,cAction,cRemark)values('" & Session("userid") & "','录入户口类型字典数据!','" & sBillCode & "')")
        End If
