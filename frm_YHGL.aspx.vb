...
Partial Class frm_YHGL
    ...
    Protected Sub but_LoadData_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles but_LoadData.Click
        ...
        If Session("cMCode") <> "" Then
            rs = cPConn.Execute("select * from DicND where zNDCode='" & Session("cMCode") & "'")
            If rs.RecordCount <> 0 Then

                'T_code.Text = rs.Fields("zNDcode").Value

                'T_tslb.Text = rs.Fields("zNDName").Value

                'Session("cMKey") = T_tslb.Text
            Else
                'T_code.Text = GetCode("NDZD", "R", Year(Now.Date), Month(Now.Date), Day(Now.Date))
    ...
    Protected Sub But_Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles But_Save.Click
        ...
        'rs = cPConn.Execute("select * from DicND where zNDcode='" & T_code.Text & "'")
        'If rs.RecordCount <> 0 Then

        '    '判断重覆关键字
        '    If Session("cMKey") <> T_tslb.Text Then
        '        re = cPConn.Execute("Select * from DicND where zNDName='" & T_tslb.Text & "'")
        ...
        '    cPConn.Execute("update DicND set zNDcode='" & T_code.Text & "',zNDName='" & T_tslb.Text & "' where zNDcode='" & T_code.Text & "'")

        '    cPConn.Execute("insert into WaterLog(cUser,cAction,cRemark)values('" & Session("userid") & "','修改年度字典数据!','" & T_code.Text & "')")
        ...
        '    sBillCode = GetCode("NDZD", "W", Year(Now.Date), Month(Now.Date), Day(Now.Date))

        '    Me.T_code.Text = sBillCode


        '    cPConn.Execute("insert into DicND(zNDCode,zNDName) values('" & sBillCode & "','" & T_tslb.Text & "')")

        '    cPConn.Execute("insert into WaterLog(cUser,cAction,cRemark)values('" & Session("userid") & "','录入年度字典数据!','" & sBillCode & "')")
        'End If
...
errhand:
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        cPConn.Execute("update dbUser set cPass='" & sMM & "' where cCode='" & Session("cMCode") & "'")
        '提示保存完毕
        Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>SaveMsg();</script>")
    End Sub
End Class
