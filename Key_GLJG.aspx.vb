...
Partial Class Key_GLJG
    ...
    Private Sub NDZD_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Session("menu") = "管理机构信息"
        Session("onelevel") = "5"
        Session("twolevel") = "5.1"

        Me.SqlDataSource1.SelectCommand = "SELECT * FROM [GLJGB]"
    ...
    Public Shared Function del(ByVal k1 As String) As String
        ...
        Else
            ...
            For i = 0 To arr.Length - 2
                ...
                If rs.RecordCount = 0 Then
                    cPConn.Execute("delete from GLJGB where Code='" & arr(i) & "'")

                    '
                    cPConn.Execute("insert into WaterLog(cUser,cAction,cRemark)values('" & HttpContext.Current.Session("userid") & "','删除管理机构数据!','" & arr(i) & "')")
                Else
                    Return "管理机构账号已使用不能删除，请在用户管理中处理后再试！"
                    Exit Function
                End If
            Next
