...
Partial Class Key_SH
    ...
    Private Sub NDZD_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Session("menu") = "信息审核"
        Session("onelevel") = "2"
        Session("twolevel") = "2.1"

        Me.SqlDataSource1.SelectCommand = "SELECT * FROM [HZXXBView]" & Session("sWhere")
    ...
    Protected Sub SH_obj_Click(sender As Object, e As EventArgs) Handles SH_obj.Click
        Dim i, j As Integer
        'Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>if(confirm('确定要删除吗？')){}else{}</script>")
        j = 0
        ...
        For i = 0 To GridView1.Rows.Count - 1
            ...
            If cbox.Checked = True Then
                Dim aa As String
                aa = Me.GridView1.Rows(i).Cells(1).Text

                cPConn.Execute("update HZXXB set SH='1' where code='" & aa & "'")

                cPConn.Execute("insert into WaterLog(cUser,cAction,cRemark)values('" & Session("userid") & "','审核信息!','" & aa & "')")
        ...
        Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>alert('本次审核通过 " & j & " 条信息！')</script>")

        Me.GridView1.DataBind()
    End Sub
    Protected Sub UNSH_obj_Click(sender As Object, e As EventArgs) Handles UNSH_obj.Click
        Dim i, j As Integer
        'Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>if(confirm('确定要删除吗？')){}else{}</script>")
        j = 0
        ...
        For i = 0 To GridView1.Rows.Count - 1
            ...
            If cbox.Checked = True Then
                ...
                cPConn.Execute("update HZXXB set SH='0' where code='" & aa & "'")

                cPConn.Execute("insert into WaterLog(cUser,cAction,cRemark)values('" & Session("userid") & "','取消审核信息!','" & aa & "')")
        ...
        Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>alert('本次取消审核 " & j & " 条信息！')</script>")
