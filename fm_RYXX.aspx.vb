...
Partial Class TG_fm_RYXX
    ...
    Protected Sub but_LoadData_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles but_LoadData.Click
        ...
        If Session("cMCode") <> "" Then
            rs = cPConn.Execute("select * from dbRYXXView where code='" & Session("cMCode") & "'")
            If rs.RecordCount <> 0 Then

                T_code.Text = rs.Fields("code").Value

                T_rycode.Text = rs.Fields("cRYCode").Value
                T_xm.Text = rs.Fields("cXM").Value
                T_lxfs.Text = rs.Fields("cLXFS").Value
                T_bz.Text = rs.Fields("cBZ").Value

                Session("cMKey") = T_xm.Text

            Else
                T_code.Text = GetCode("RYXX", "R", Year(Now.Date), Month(Now.Date), Day(Now.Date))
    ...
    Protected Sub But_Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles But_Save.Click
        ...
        T_rycode.Text = Trim(Replace(T_rycode.Text, "'", ""))
        T_xm.Text = Trim(Replace(T_xm.Text, "'", ""))
        T_lxfs.Text = Trim(Replace(T_lxfs.Text, "'", ""))
        T_bz.Text = Trim(Replace(T_bz.Text, "'", ""))

        If T_rycode.Text = "" Then
            Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>alert('请录入人员编号！')</script>")
            T_rycode.Focus()
            Exit Sub
        ElseIf T_xm.Text = "" Then
            Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>alert('请录入姓名！')</script>")
            T_xm.Focus()
            Exit Sub
            'ElseIf T_lxfs.Text = "" Then
            '    Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>alert('请录入联系方式！')</script>")
            '    T_lxfs.Focus()
            '    Exit Sub
        ...
        rs = cPConn.Execute("select * from dbRYXX where code='" & T_code.Text & "'")
        If rs.RecordCount <> 0 Then

            '判断重覆关键字
            If Session("cMKey") <> T_xm.Text Then
                re = cPConn.Execute("Select * from dbRYXX where cXM='" & T_xm.Text & "'")
            ...
            cPConn.Execute("update dbRYXX set code='" & T_code.Text & "',cRYCode='" & T_rycode.Text & "',cXM='" & T_xm.Text & "',cLXFS='" & T_lxfs.Text & "',cBZ='" & T_bz.Text & "' where code='" & T_code.Text & "'")

            cPConn.Execute("insert into WaterLog(cUser,cAction,cRemark)values('" & Session("userid") & "','修改人员信息','" & T_code.Text & "')")
        ...
        Else
            ...
            sBillCode = GetCode("RYXX", "W", Year(Now.Date), Month(Now.Date), Day(Now.Date))

            Me.T_code.Text = sBillCode


            cPConn.Execute("insert into dbRYXX(code,cRYCode,cXM,cLXFS,cBZ) values('" & sBillCode & "','" & T_rycode.Text & "','" & T_xm.Text & "','" & T_lxfs.Text & "','" & T_bz.Text & "')")

            cPConn.Execute("insert into WaterLog(cUser,cAction,cRemark)values('" & Session("userid") & "','录入人员信息','" & sBillCode & "')")
        End If
